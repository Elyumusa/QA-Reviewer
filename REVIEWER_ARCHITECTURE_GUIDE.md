...
Malformed or truncated model output is retried once with the configured larger output allowance and an exact explanation. Parseable global-map or synthesis JSON with a narrow schema defect goes directly to a targeted non-thinking repair request, avoiding another full-context call. If coverage retry output is malformed JSON, the generic JSON client passes its raw text to a bounded coverage-specific repair request. An irreparable coverage response becomes a conservative empty coverage result with explicit limitations so final synthesis can still consolidate completed chunk evidence; transport and permanent API failures remain real failures. Recoverable fetch/socket/connect and DNS failures receive error-specific bounded retries. HTTP 429 and temporary 5xx responses are retried, while permanent 4xx responses such as a missing key or invalid parameters are not.
